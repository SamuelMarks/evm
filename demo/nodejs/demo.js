...
EVMDAG1Client = require('./evm-dag1-client.js');
...
function DemoNode(name, host, port) {
    this.name = name
    this.api = new EVMDAG1Client(host, port)
    this.accounts = {}
}
...
"The private keys reside directly on the evm-dag1 nodes. In a production \n" +
...
"EVM-DAG1 converted the transaction into raw bytes, signed it and submitted \n" +
"it to DAG1 for consensus ordering. DAG1 gossiped the raw transaction to \n" +
"the other DAG1 nodes which ran it through the consensus algorithm until they \n" +
"were each ready to commit it back to EVM-DAG1. So each node received and \n" +
...
"As an exercise you can check that the transaction was run through every DAG1 \n" +
